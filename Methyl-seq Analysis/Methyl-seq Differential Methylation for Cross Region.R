...
disorder = args[[1]]
chr.num = args[[2]]
cat("disorder =", disorder, ", chr.num =", chr.num)
...
disorder_chr = paste(disorder, paste0("chr", chr.num), sep = "_")
...
sampleSets <- list(MDD = unlist(metadata.df[which(metadata.df$DxGroup1 == "Control" | metadata.df$DxGroup1 == "MDD"), "SampleID"]),
                   PTSD = unlist(metadata.df[which(metadata.df$DxGroup1 == "Control" | metadata.df$DxGroup1 == "PTSD"), "SampleID"]))

samples <- sampleSets[[disorder]]
...
metadata.analysis <- metadata.analysis[, c("DxGroup1", "RegionSci", "Age", "Sex", "PMI", "Source", "Smoking", "NeuPos", "AFR", "EUR", "SAS")]
...
dml.fit <- DMLfit.multiFactor(bsdata, metadata.analysis,
                              formula = as.formula("~DxGroup1 + RegionSci + Age + Sex + PMI + Source + Smoking + NeuPos + AFR + EUR"),
                              smoothing = FALSE)
# extract dml results
coef <- paste0("DxGroup1", disorder)
...
res_dir = "res_dir"
save(dml.results, file = paste0(res_dir, "DSS_Filtered_NoSmooth_Ancestry_Prop_ALLREGION_Results_", disorder_chr, ".Rdata"))
